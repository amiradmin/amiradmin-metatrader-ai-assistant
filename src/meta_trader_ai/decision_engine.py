...
def _candidate_from_scores(buy_score: float, sell_score: float, min_edge: float) -> Decision:
    edge = buy_score - sell_score
    if abs(edge) < min_edge:
        return Decision.WAIT
    return Decision.BUY if edge > 0 else Decision.SELL
...
def _safety(snapshot: MarketSnapshot, config: StrategyConfig) -> list[SafetyGate]:
    ...
    gates.append(SafetyGate(name="spread", passed=spread_ok, reason=f"spread={snapshot.spread_points} points; limit={config.safety.max_spread_points}"))

    news_ok = True
    if config.safety.block_high_news and snapshot.news_risk == "HIGH":
        news_ok = False
    if config.safety.block_unknown_news and snapshot.news_risk == "UNKNOWN":
        news_ok = False
    ...
    price_ok = snapshot.ask > snapshot.bid > 0 and snapshot.point > 0
    gates.append(SafetyGate(name="market_data", passed=price_ok, reason="bid/ask/point sanity check"))
    return gates
...
def build_decision(snapshot: MarketSnapshot, config: StrategyConfig, performance: PerformanceSummary | None = None) -> DecisionResponse:
    ...
    candidate = _candidate_from_scores(buy_score, sell_score, config.decision.min_side_edge)
    ...
    for item in raw:
        ...
        if candidate == Decision.BUY:
            ...
            score = item.sell
            reason = item.sell_reason
        else:
            score = max(item.buy, item.sell)
    ...
    if candidate == Decision.WAIT:
        blockers.append(f"side edge {abs(buy_score - sell_score):.1f} < {config.decision.min_side_edge:.1f}")
    else:
    ...
    return DecisionResponse(signal_id=signal_id, generated_at=datetime.now(timezone.utc), symbol=snapshot.symbol, timeframe=snapshot.timeframe, candidate=candidate, decision=decision, trade_allowed=trade_allowed, buy_score=round(buy_score, 2), sell_score=round(sell_score, 2), side_edge=round(abs(buy_score - sell_score), 2), passed_count=passed_count, min_pass_count=config.decision.min_pass_count, blockers=blockers, factors=factors, safety=safety, performance=performance or PerformanceSummary())
