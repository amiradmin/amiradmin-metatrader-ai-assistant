...
def build_decision(snapshot: MarketSnapshot, config: StrategyConfig, performance: PerformanceSummary | None = None) -> DecisionResponse:
    ...
    for item in raw:
        ...
        if candidate == Decision.BUY:
            ...
            score, reason = max(item.buy, item.sell), "no directional edge"
    ...
    return DecisionResponse(signal_id=signal_id, generated_at=datetime.now(timezone.utc), symbol=snapshot.symbol, timeframe=snapshot.timeframe, candidate=candidate, decision=decision, trade_allowed=trade_allowed, buy_score=round(buy_score, 2), sell_score=round(sell_score, 2), side_edge=round(abs(buy_score - sell_score), 2), passed_count=passed_count, min_pass_count=config.decision.min_pass_count, blockers=blockers, primary_blocker=blockers[0] if blockers else None, factors=factors, safety=safety, performance=performance or PerformanceSummary())
